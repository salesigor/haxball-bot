...
const blacklistconn = ['3137372E35372E3135302E313736', '3136372E3234392E39332E313135', '3137372E37362E3232342E3730']; // Arthur MM, ᱦiᱮ∀Ʀd, Schneider
const cartaoamarelo = ['3138392E38352E32392E3739', '3138392E38352E32392E3733']; // 𝘿𝙄𝘼𝙕
var blacklist = [{
  Nick: "Arthur MM",
  Auth: "YD0Jm8MmB9G9YJCwJEEoIcC1SvD3Q2811xT9T-NTmVw",
  Conn: "3137372E35372E3135302E313736"
}, {
  Nick: "ᱦiᱮ∀Ʀd",
  Auth: "YJINpE0p70-sAT-nTx10vI5VvHyQ-jkZ1C-zi6dI2us",
  Conn: "3136372E3234392E39332E313135"
}, {
  Nick: "Schneider",
  Auth: "_nsAyl-n61ELIOJghIDiHojx-parT0N19K374jqygqo",
  Conn: "3137372E37362E3232342E3730"
}, {
  Nick: "",
  Auth: "",
  Conn: ""
...
let regex = ["fdp", "cu", "carai", "cuzao", "porra", "arrombado", "cu preto", "lixo", "autista", "lixeira", "verme", "Horrível", "seu merda", "filho da puta", "caralho", "seu gordo", "cuzão", "vadia", "sua mãe", "seu fdp", "cala a boca", "puta", "fudido", "krl", "f d p", "vtnc", "vai tomar no cu", "crl", "cadeirante", "caderante"];
...
function nameForbid(player) {
  if (forbid.includes(player.name)) {
    room.kickPlayer(player.id, 'nick proibido nessa sala', true);
  }
}
;
function banBlackListed(player) {
  if (blacklistconnID.includes(player.id)) {
    room.kickPlayer(player.id, 'Você está na BLACKLIST', true);
  }
}
;
...
const CounterBot = 'MTEyMDQyNzUzMzEwODc3NzA1Mg.GkypKH.DgiYMDj4j4qhxxSAD4rTskggkqpFPiARe8DIMI';

// Função para armazenar dados de hat-tricks no canal do Discord
async function storeHatTrick(player) {
  const channelId = '1120398298654707832';
  const channel = client.channels.cache.get(channelId);
  const playerName = player.name;
  const key = `hattrick_${playerName}`;
  // Obtém a quantidade atual de Hat Tricks do jogador
  const currentHatTricks = await getHatTrick(key);
  // Incrementa a quantidade de Hat Tricks em 1
  const newHatTricks = (currentHatTricks ? parseInt(currentHatTricks) : 0) + 1;
  // Envia mensagem no Discord informando o Hat Trick feito pelo jogador
  channel.send(`O jogador ${playerName} fez um Hat Trick! Total de Hat Tricks: ${newHatTricks}`);
  // Armazena o novo Hat Trick no Discord
  channel.send(`!store ${key} ${newHatTricks}`);
}
;

// Função para recuperar a quantidade de Hat Tricks de um jogador do canal do Discord
async function getHatTrick(player) {
  const channelId = 'INSIRA_AQUI_O_ID_DO_CANAL'; // Substitua pelo ID do canal criado no Discord
  const channel = client.channels.cache.get(channelId);
  const playerName = player.name;
  const key = `hattrick_${playerName}`;
  const messages = await channel.messages.fetch();
  const dataMessage = messages.find(msg => msg.content.startsWith(`!store ${key}`));
  if (dataMessage) {
    const value = dataMessage.content.split(' ')[2];
...
function sendAnnouncementToDiscord(message) {
  ...
  var params = {
    avatar_url: 'https://cdn.discordapp.com/icons/1108404626014871633/39f9e5007c336295e78937769194251a.png?size=2048',
    // Avatar WEBHOOK
    username: 'Info. Players',
    // Nome WEBHOOK
    content: message
  };
...
function sendRoomLinkToDiscord(message) {
  ...
  request.setRequestHeader('Content-type', 'application/json');
  var params = {
    avatar_url: 'https://cdn.discordapp.com/attachments/1113830556967379064/1113830882818666588/image.png',
    // Avatar WEBHOOK
    username: 'Malco',
    // Nome WEBHOOK
    content: message
...
function sendScoresToDiscord(message) {
  ...
  request.setRequestHeader('Content-type', 'application/json');
  var params = {
    avatar_url: 'https://cdn.discordapp.com/attachments/1113830556967379064/1113881527181398087/image.png',
    // Avatar WEBHOOK
    username: 'Soberbo',
    // Nome WEBHOOK
    content: message
...
function sendHattricksToDiscord(message) {
  ...
  request.setRequestHeader('Content-type', 'application/json');
  var params = {
    avatar_url: 'https://cdn.discordapp.com/attachments/1113830556967379064/1119854937241813002/image.png',
    // Avatar WEBHOOK
    username: 'Hat Trick Counter',
    // Nome WEBHOOK
    content: message
  };
  request.send(JSON.stringify(params));
...
function getPlayersGoalCount() {
  if (teamR.length == 1) {
    if (lastPlayersTouched[0].id == teamR[0].id) {
      goalsRp1++;
    }
  }
  if (teamR.length == 2) {
    ...
    if (lastPlayersTouched[0].id == teamR[1].id) {
      goalsRp2++;
    }
  }
  if (teamR.length == 3) {
    ...
    if (lastPlayersTouched[0].id == teamR[2].id) {
      goalsRp3++;
    }
  }
  if (teamB.length == 1) {
    if (lastPlayersTouched[0].id == teamB[0].id) {
      goalsBp1++;
    }
  }
  if (teamB.length == 2) {
    ...
    if (lastPlayersTouched[0].id == teamB[1].id) {
      goalsBp2++;
    }
  }
  if (teamB.length == 3) {
    ...
    if (lastPlayersTouched[0].id == teamB[2].id) {
      goalsBp3++;
    }
  }
}
...
function getPlayersAssistCount() {
  if (teamR.length == 1) {
    if (lastPlayersTouched[1].id == teamR[0].id) {
      assistsRp1++;
    }
  }
  if (teamR.length == 2) {
    ...
    if (lastPlayersTouched[1].id == teamR[1].id) {
      assistsRp2++;
    }
  }
  if (teamR.length == 3) {
    ...
    if (lastPlayersTouched[1].id == teamR[2].id) {
      assistsRp3++;
    }
  }
  if (teamB.length == 1) {
    if (lastPlayersTouched[1].id == teamB[0].id) {
      assistsBp1++;
    }
  }
  if (teamB.length == 2) {
    ...
    if (lastPlayersTouched[1].id == teamB[1].id) {
      assistsBp2++;
    }
  }
  if (teamB.length == 3) {
    ...
    if (lastPlayersTouched[1].id == teamB[2].id) {
      assistsBp3++;
    }
  }
}
...
function checkTime() {
  ...
  if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit != 0) {
    ...
    goldenGoal = true;
    var messages = [drawTimeLimit * 60 + " segundos para o final!", "Vou dar " + drawTimeLimit * 60 + " segundos!"];
    var randomIndex = Math.floor(Math.random() * messages.length);
    var announcement = messages[randomIndex];
    room.sendAnnouncement(centerText(announcement), null, yellow, "normal");
    var messagens = ["⚽ Primeiro GOL vence! ⚽", "⚽ GOL DE OURO ativado ⚽"];
...
function updateStats() {
  // Função atualizar STATUS a cada fim de jogo.
...
;
function findGK() {
  // Função que procura o GK da partida.
  var tab = [[-1, ""], [-1, ""]];
  for (var i = 0; i < extendedP.length; i++) {
...
room.onPlayerJoin = function (player) {
  ...
  var messages = ["👋 Salve, " + player.name + "!", "👋 Eae, " + player.name + "!"];
...
room.onPlayerChat = function (player, message) {
  ...
  if (["!me"].includes(message[0].toLowerCase())) {
    // mostra suas atuais estatisticas, somente para você.
  ...
  if (["!stats"].includes(message[0].toLowerCase())) {
    // mostra suas atuais estatisticas, mostra para todos da sala.
    room.sendAnnouncement(centerText("Esta função ainda não está disponível. Lamento"), player.id, warn, "italic");
    /*
    var stats;
    localStorage.getItem(getAuth(player)) ? stats = JSON.parse(localStorage.getItem(getAuth(player))) : stats = [0, 0, 0, 0, "0.00", 0, 0, 0, 0, "0.00"]; 
  ...
  if (["gk"].includes(message[0].toLowerCase())) {
    ...
    if (GKListname.includes(player.name)) {} else {
      GKListname.push(" " + player.name);
    }
  }
  ...
  if (["!ain", "ain"].includes(message[0].toLowerCase())) {
    var messages = ["AINN, PAI PARAAA!", "AIN, ZÉ DA MANGA!", "AIN, ZÉ DA MANGA!", "AIN, ZÉ DA MANGA!"];
    var randomIndex = Math.floor(Math.random() * messages.length);
    var announcement = messages[randomIndex];
  ...
  if (["!gol", "gol"].includes(message[0].toLowerCase())) {
    var messages = ["LAAAAÇO! ⚽", "GOLAAAAÇO! ⚽", "É GOOOOL! ⚽", "OLHA O GOL, OLHA O GOL, OLHA O GOL!"];
    var randomIndex = Math.floor(Math.random() * messages.length);
    var announcement = messages[randomIndex];
  ...
  if (["!chupa", "chupa"].includes(message[0].toLowerCase())) {
    var messages = ["Chupaaa!", "Chupaaa!", "Chupaaa!", "Chupaaa!", "Chupaaa!", "Chupaaa, Lady Gaga! 👱‍♀️", "Chupaaa, imperatriz! 🧞‍♂️"];
    var randomIndex = Math.floor(Math.random() * messages.length);
    var announcement = messages[randomIndex];
    room.sendAnnouncement(player.name + ": " + announcement, null, white, "bold");
    return false;
  ...
  if (["size"].includes(message[0].toLowerCase())) {
    if (player.admin) {
      if (message[1] == "r1") {
        if (message[2] == "big") {
          room.setPlayerDiscProperties(teamR[0].id, {
            radius: 17,
            invMass: 30 / 30
          });
        ...
        if (message[2] == "big") {
          room.setPlayerDiscProperties(teamR[1].id, {
            radius: 17,
            invMass: 30 / 30
          });
        } else if (message[2] == "normal") {
        ...
        if (message[2] == "big") {
          room.setPlayerDiscProperties(teamR[2].id, {
            radius: 17,
            invMass: 30 / 30
          });
        } else if (message[2] == "normal") {
        ...
        if (message[2] == "big") {
          room.setPlayerDiscProperties(teamB[0].id, {
            radius: 17,
            invMass: 30 / 30
          });
        } else if (message[2] == "normal") {
        ...
        if (message[2] == "big") {
          room.setPlayerDiscProperties(teamB[1].id, {
            radius: 17,
            invMass: 30 / 30
          });
        } else if (message[2] == "normal") {
        ...
        if (message[2] == "big") {
          room.setPlayerDiscProperties(teamB[2].id, {
            radius: 17,
            invMass: 30 / 30
          });
        } else if (message[2] == "normal") {
...
room.onGamePause = function (byPlayer) {
  ...
  choose = true;
  setTimeout(function () {
    var messages = ["Opa, vamos ter que levar para o VAR analizar...", "VAR está coferindo o lance..."];
    var randomIndex = Math.floor(Math.random() * messages.length);
    var announcement = messages[randomIndex];
...
room.onGameUnpause = function (byPlayer) {
  room.sendAnnouncement(centerText("Choose Mode Desativado"), null, green, "bold");
  choose = false;
  var messages = ["⚽ Pro jogo! ⚽", "⚽ E o Juiz apita! ⚽"];
  var randomIndex = Math.floor(Math.random() * messages.length);
  var announcement = messages[randomIndex];
  room.sendAnnouncement(centerText(announcement), null, white, "bold");
};
...
room.onStadiumChange = function (newStadiumName, byPlayer) {
  if (byPlayer && byPlayer.id != 0) {
    ...
    var messages = ["Esse é o estádio que vai dar jogo.", "Mapa mudou, agora da jogão!", "Santiago Bernabéu? Não, aqui é futsal!"];
    var randomIndex = Math.floor(Math.random() * messages.length);
    var announcement = messages[randomIndex];
    room.sendAnnouncement(centerText(announcement), null, yellow, "bold", 0);
  }
